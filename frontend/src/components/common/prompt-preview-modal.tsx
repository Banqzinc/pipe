...
import { usePreviewPrompt, usePreviewStackPrompt } from '../../api/mutations/preview-prompt.ts';
...
export function PromptPreviewModal({
  ...
  const previewStackPrompt = usePreviewStackPrompt();
  ...
  const isStackPreview = !!stackId;
  const activePreview = isStackPreview ? previewStackPrompt : previewPrompt;
  ...
  useEffect(() => {
    if (isOpen) {
      if (isStackPreview && stackId) {
        previewStackPrompt.mutate(stackId, {
          onSuccess: (data) => {
  ...
  const summary = activePreview.data?.context_summary;
  ...
  const title = isStackPreview
    ? `Customize & Run Stack Review`
    : 'Customize & Run Review';

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title}>
      <div className="px-6 py-4 space-y-4">
        {/* Linear + Notion inputs — only for single PR */}
        {!isStackPreview && (
        ...
        {summary && (
          <div className="flex flex-wrap gap-3 text-xs text-gray-500">
            {isStackPreview && summary.stack_size != null && (
              <span>Stack Review: {summary.stack_size} PRs</span>
            )}
            {summary.has_linear_ticket && <span>Linear ticket linked</span>}
            {summary.has_notion_url && <span>Notion proposal linked</span>}
            {!isStackPreview && summary.stack_position != null && summary.stack_size != null && (
        ...
        {activePreview.error && (
          <div className="rounded-lg border border-red-800 bg-red-500/10 p-3 text-red-400 text-sm">
            {activePreview.error instanceof Error
              ? activePreview.error.message
              : 'Failed to build prompt.'}
          </div>
        )}

        {/* Prompt editor */}
        {!activePreview.isPending && !activePreview.error && (
          <>
            <textarea
              value={editedPrompt}
      ...
      <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-800">
        ...
        <button
          type="button"
          onClick={handleRun}
          disabled={isRunning || activePreview.isPending || !editedPrompt || updatePr.isPending}
